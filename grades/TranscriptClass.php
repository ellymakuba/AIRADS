...
class bus_report {
	...
	function bus_report($class,$student,$mp_array, $db) {
		$this->subject = $this->get_subject($db);

		$this->scheduled_subjects = $this->get_scheduled_subjects($class,$student,$mp_array,$db);
	}
	...
	function get_scheduled_subjects($class,$student,$mp_array,$db) {
		

		$scheduled_subjects_array = array();
	$sql = "select rs.id, rs.subject_id from registered_students rs,subjects sub,collegeperiods cp,studentsmarks sm,markingperiods mp 
		WHERE cp.id IN ($mp_array)
		AND sm.exam_mode=mp.id
		AND rs.id=sm.calendar_id
		AND mp.title LIKE 'TRANSCRIPT'
		AND cp.term_id=rs.term
		AND cp.year=rs.year
		AND rs.student_id='$student'
		AND sub.id=rs.subject_id 
		AND rs.class_id='$class'
		GROUP BY rs.subject_id,sm.calendar_id";
		//echo $query;
		$result = DB_query($sql,$db);
...
function total_marks($class,$student_id,$calendar_id, $subject_id,$db) {
		$sql = "select marks as smarks from studentsmarks sm
