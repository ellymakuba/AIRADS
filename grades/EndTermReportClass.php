...
class bus_report {
	...
	function bus_report($student,$term,$year, $db) {
		$this->subject = $this->get_subject($db);

		$this->scheduled_subjects = $this->get_scheduled_subjects($student,$term,$year,$db);
	}
	...
	function get_scheduled_subjects($student,$term,$year,$db) {
		

		$scheduled_subjects_array = array();
		$sql = "select id, subject_id from registered_students WHERE term='$term'
		AND year='$year'
...
function total_marks($subject_id,$student_id,$term,$year,$calendar_id,$db) {
