...
echo '</table>';
echo '<table class="enclosed">';
if(!isset($_SESSION['classSession']) && !isset($_SESSION['period']))
{
...
if (isset($_POST['register'])) 
{
	...
	else if($_POST['period'] ==0){
	$feeStructureVariableErrors=1;
		prnMsg( _('Please select period you want to create fee structure'),'error');
	}
...
{	
	...
	{
		$sql2="SELECT stockmaster.actualcost,stockmaster.id,stockmaster.description  FROM stockmaster
...
if (DB_num_rows($SearchResult)==1)
{
  ...
  $sql = "SELECT stockmaster.actualcost,stockmaster.id,stockmaster.description,stockmaster.id FROM stockmaster
	...
	{	
		...
		{  			
			$_SESSION['InvoiceItems']->add_to_cart ($myrow['id'],$NewItemQty,$myrow['description'],$myrow['actualcost']
			,0,1,1,1,0,1,Date($_SESSION['DefaultDateFormat']),0,1,1,1,1,'','No',-1,1,'','','',1);
		}
...
if (count($_SESSION['InvoiceItems']->LineItems)>0)
{
	 ...
	 echo '<input type="hidden" name="FormID" value="' . $_SESSION['FormID'] . '" />';
	$LineNumber = $_SESSION['InvoiceItems']->LineCounter;
	$sql = 'SELECT class_name FROM classes WHERE id="'.$_SESSION['classSession'].'"';
	$result=DB_query($sql,$db);
	$myrow=DB_fetch_array($result);
...
echo '<tr><td colspan="6" style="text-align:center;"><h1><b>'.$class_session.' '.$period_session.' '._('Fee Structure Items') . '</b></h3></td></tr>';
...
if(isset($_POST['submitInvoice'])){
		...
		VALUES ('" .$_SESSION['classSession'] ."','" .$_SESSION['period'] ."') ";
