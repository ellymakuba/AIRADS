...
if(isset($_REQUEST['student_id']))
{
	...
	$_SESSION['studentClassSession'] = $myrow['class_id'];
}
...
echo '<tr><td>' . _('Period') . ":</td><td><select name='period'>";
...
if (isset($_POST['loadFeeStructure'])) {	
    ...
    $sql = "SELECT ab.* FROM autobilling ab
    		...
    		{
    			prnMsg(_($_SESSION['student'].' The fee structure for this class has not been created for this terms.'),'warn');
    	        exit();
    	    }
}
...
if ($_SESSION['studentClassSession'] > 0 && isset($_SESSION['period']) && !isset($_SESSION['invoice_id'])){
    $_SESSION['classFeeStructure'] = new Cart;
    $sql = "SELECT id FROM autobilling WHERE term_id='" . $_SESSION['period'] . "' AND class_id='" . $_SESSION['studentClassSession'] . "'";
    $result= DB_query($sql, $db);
    $myrow = DB_fetch_array($result);
        ...
        if (DB_fetch_row($result_exist) > 0) {
		   ...
		   '" . $_SESSION['period'] . "')";
            ...
            foreach ($_POST['id'] as $value) {                
                $sql    = "INSERT INTO invoice_items ( invoice_id,product_id,unitprice,priority) 
