...
echo "<form method='post' action=" . $_SERVER['PHP_SELF'] . '>';
	...
	echo '<tr><td>' . _('Search UserName') . ':<input type="Text" name="searchval"  size=30   maxlength=20></td><td>
	<input  type="submit" name="form1" value="submit"></td></tr>';
...
$limit = 'LIMIT ' .($pageno - 1) * $rows_per_page .',' .$rows_per_page;
 ...
 if (isset($_POST['form1']))
 {
	...
	FROM www_users WHERE userid LIKE  '". $SearchString."' OR realname LIKE  '". $SearchString."'";
	$result = DB_query($sql,$db);
 }
	else
 {
	$sql = 'SELECT userid,realname,phone,email,salesman,lastvisitdate,fullaccess,pagesize,theme,language
	FROM www_users $limit';
	$result = DB_query($sql,$db);
  }
