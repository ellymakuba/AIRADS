...
if (!isset($_GET['delete'])) {
	...
	if (isset($SelectedAccount)) {
		...
		echo "<table class=enclosed><tr><td>" . _('Account Code') . ":</td><td>" . $_POST['AccountCode'] . "</td></tr>";
	} else {
		echo "<table class=enclosed>";
		echo "<tr><td class=\"visible\">" . _('Account Code') . ":</td><td class=\"visible\"><input type=TEXT name='AccountCode' size=11 class=number maxlength=10></td></tr>";
	}

	if (!isset($_POST['AccountName'])) {$_POST['AccountName']='';}
	echo '<tr><td class="visible">' . _('Account Name') . ":</td><td class=\"visible\"><input type='Text' size=51 maxlength=50 name='AccountName' value='" . $_POST['AccountName'] . "'></td></tr>";

	$sql = 'SELECT groupname FROM accountgroups ORDER BY sequenceintb';
	$result = DB_query($sql, $db);

	echo '<tr><td class="visible">' . _('Account Group') . ':</td><td class="visible"><select name=Group>';
