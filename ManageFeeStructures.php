...
echo '<th>'._('Class').'</th><th>'._('Period').'</th></tr>';   
...
$sql = "SELECT ab.*,cl.class_name,t.title,years.year FROM autobilling ab
INNER JOIN classes cl ON cl.id=ab.class_id
INNER JOIN collegeperiods cp ON cp.id=ab.term_id
...
while ($row = DB_fetch_array($result))
{
	...
	<td><?php 
	if($administrator_rights ==8){
	echo '<a  href="' . $rootpath . '/FeeStructure.php?feeStructure=' . $row['id'].'&class='.$row['class_id'].'">'.$row['class_name'].'</a>';
	}
	?></td><?php
